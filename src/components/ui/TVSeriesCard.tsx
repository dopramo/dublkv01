...
export default function TVSeriesCard({ series, fill = false }: TVSeriesCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative rounded-xl overflow-hidden bg-dark-800 shadow-lg group-hover:shadow-2xl group-hover:shadow-brand-500/20 transition-all duration-500 border border-white/5"
      ...
      >
        ...
        {/* TV Series Badge */}
        <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-md bg-brand-500/90 backdrop-blur-sm shadow-md">
          <span className="text-[10px] font-extrabold text-white tracking-wider uppercase">TV</span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-7 h-7 rounded-full bg-[#00ff73] text-black flex items-center justify-center shadow-xl">
              <svg className="w-3.5 h-3.5 ml-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" />
              </svg>
            </div>
            <span className="text-[11px] text-white font-semibold">Watch Series</span>
          </div>
      ...
      <div className="mt-2 px-0.5" style={{ height: '40px', overflow: 'hidden' }}>
        <h3 className="text-xs font-medium text-dark-200 group-hover:text-[#00ff73] transition-colors line-clamp-2 leading-snug">
          {series.title}
        </h3>
        {series.release_year && (
          <p className="text-[10px] text-dark-500 mt-0.5">{series.release_year}</p>
        )}
      </div>
    </Link>
  );
